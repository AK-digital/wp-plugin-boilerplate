...
$PLUGIN_NAME_TITLECASE = (Get-Culture).TextInfo.ToTitleCase($PLUGIN_NAME_UNDERSCORE.ToLower())
...
$Content = $Content -replace "WordPress Plugin Boilerplate", $PluginName `
    ...
    -replace "plugin-name", $TextDomain `
...
Function Replace-Text {
    ...
    Get-ChildItem -Path $directory -Recurse -File | ForEach-Object {
        ...
        try {
            ...
            # Replace text using the -replace operator
            $content = $content -replace [regex]::Escape('Plugin_Name'), $PLUGIN_NAME_TITLECASE `
                                -replace [regex]::Escape('plugin_name'), $PLUGIN_NAME_UNDERSCORE `
                                -replace [regex]::Escape('plugin-name'), $PLUGIN_NAME_HYPHEN
