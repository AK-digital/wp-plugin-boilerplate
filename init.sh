...
PLUGIN_NAME_CLASSCASE=$(echo "$PLUGIN_NAME_UNDERSCORE" | awk -F"_" '{for(i=1;i<=NF;i++) $i=toupper(substr($i,1,1)) tolower(substr($i,2));}1' OFS="_")
...
Content=$(echo "$Content" | \
    sed -e "s|WordPress Plugin Boilerplate|$PluginName|g" \
        -e "s|http://example.com/plugin-name-url/|$PluginURI|g" \
...
rename_files() {
    local directory=$1
    find "$directory" -type f -name "*plugin-name*" | while read file; do
